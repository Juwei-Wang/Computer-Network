...
void printmenu()
  {
    ...
    printf("  1 - Enter a word or a sentence\n");
...
int main(int argc, char* argv[])
  {
    ...
    //printmenu();
    printf("please input data soure\n");
    //scanf("%d", &choice);
    //scanf("%s", input);
	    len = 0;
	    while( (c = getchar()) != '\n' )
    ...
    while(1)
      {
	    ...
	    printf("Enter your transformation: \n");
      ...
      printf("my transofrm: %s\n", message);
	    ...
	    if( (bytes = recv(sockfd, messageback, MAX_WORD_LENGTH - 1, 0)) > 0 )
	      {
		/* make sure the message is null-terminated in C */
		messageback[bytes] = '\0';
