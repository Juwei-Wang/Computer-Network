    ...
    while (1){
        ...
        while ((clientBytes = recv(data_sock, client_request, MESSAGE_SIZE, 0)) > 0){
            ...
            while((serverBytes = recv(web_sock, server_response, MESSAGE_SIZE, 0)) > 0){
                ...
                content.append(server_response, serverBytes);
            ...
            // content is used to store final request and reply
            printf("origin:\n%s\n", content.c_str());

            //eo for replacing the link
            std::regex e0("(<a.*href=)(\".*Floppy.*\")");
            content = std::regex_replace(content, e0, "$1\"http://pages.cpsc.ucalgary.ca/~carey/CPSC441/trollface.jpg\"");
            printf("e0:\n%s\n", content.c_str());
            //e1 for replacing the pictures
            std::regex e1("(<img.*src=)(\".*Floppy.*?\")");
            content = std::regex_replace(content, e1, "$1\"http://pages.cpsc.ucalgary.ca/~carey/CPSC441/trollface.jpg\"");
            //e2 for replacing the floppy by trolly
            std::regex e2("Floppy");
            content = std::regex_replace(content, e2, "Trolly");
            printf("e2:\n%s\n", content.c_str());
            //e3 for replacing the Italy by japan
            std::regex e3("Italy");
            content = std::regex_replace(content, e3, "Japan");
            printf("e3:\n%s\n", content.c_str());

            size_t send_len = content.length();
            int sended_size = 0;
            size_t index = 0;
            while (send_len > 0) {
                if ((sended_size = send(data_sock, content.c_str() + index, send_len, 0)) < 0) {
                    perror("send() call failed...\n");
                    break;
                } else {
                    index = index + sended_size;
                    send_len = send_len - sended_size;
                }
            }

        }//while recv() from client
        //close(data_sock);
        shutdown(data_sock, SHUT_RD);
    }//infinite loop
    return 0;
}
